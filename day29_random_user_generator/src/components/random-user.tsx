...
const RandomUser: React.FC = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-blue-200 text-foreground">
      ...
      <Button onClick={fetchRandomUser} className="mb-6 bg-slate-700 hover:bg-slate-500">
        Fetch New User
      </Button>
      {loading && (
      ...
      {user && (
        <Card className="border-0 shadow-lg rounded-lg overflow-hidden max-w-sm relative bg-slate-200">
          <CardHeader className="h-32 bg-slate-700 relative">
            <Image
              src={user.image}
              alt={user.name}
              width={80}
