...
export default function RecipeSearch() {
  ...
  const handleSearch = async (event: FormEvent) => {
    ...
    try {
      const response = await fetch(
        `https://api.edamam.com/search?q=${query}&app_id=0afab4e7&app_key=1d7fb3355270b960fb149e1782d980c4`
  ...
  return (
    <div className="flex flex-col h-full  w-full max-w-6xl mx-auto p-4 md:p-6 bg-yellow-400">
      {/* Header section */}
      <header className="flex flex-col items-center mb-6">
        <h1 className="text-3xl font-bold mb-2">✌𝓡𝓮𝓬𝓲𝓹𝓮 𝓢𝓮𝓪𝓻𝓬𝓱✌</h1>
        <p className="text-lg mb-4">
          <span className = "">Find delicious recipes by ingredients you have at home.</span>
        </p>
        {/* Example search terms */}
        <div className="mb-4 ">
          <p >Try searching for:</p>
          <div className="flex space-x-2 bg-green900">
        ...
        <form className="relative w-full max-w-md mb-6 " onSubmit={handleSearch}>
          ...
          <Button
            ...
            variant="ghost"
            size="icon"
            className="absolute right-2 top-1/2 -translate-y-1/2 bg-yellow-500 "
          >
      ...
      {/* Loading spinner */}
      {loading ? (
        <div className="flex flex-col justify-center items-center w-full h-full bg-yellow-500">
          ...
          {recipes.map((recipe) => (
            <Card className="group relative" key={recipe.uri}>
              <Image
                ...
                width={400}
                height={300}
                className="rounded-t-lg object-cover w-full h-48 group-hover:opacity-50 transition-opacity bg-yellow-500"
              />
